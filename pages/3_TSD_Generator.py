...
for idx, reqs in enumerate(st.session_state.api_requirements):
    with st.expander(f"API Spec #{idx + 1}", expanded=True):
        st.session_state.api_requirements[idx]["service_name"] = st.text_input(f"Service Name #{idx + 1}",
                                                                               value=reqs["service_name"],
                                                                               key=f"svc_{idx}")
        st.session_state.api_requirements[idx]["description"] = st.text_input(f"Description #{idx + 1}",
                                                                              value=reqs["description"],
                                                                              key=f"desc_{idx}")
        st.session_state.api_requirements[idx]["inputs"] = st.text_input(f"Inputs #{idx + 1}", value=reqs["inputs"],
                                                                         key=f"inp_{idx}")
        st.session_state.api_requirements[idx]["outputs"] = st.text_input(f"Outputs #{idx + 1}", value=reqs["outputs"],
                                                                          key=f"out_{idx}")
        st.session_state.api_requirements[idx]["process"] = st.text_area(f"Process #{idx + 1}", value=reqs["process"],
                                                                         key=f"proc_{idx}")
...
for idx, spec in enumerate(st.session_state.api_specs):
    with st.expander(f"API Spec #{idx + 1}", expanded=True):
        st.session_state.api_specs[idx]["service_name_spec"] = st.text_input(f"Service Name #{idx + 1}",
                                                                             value=spec["service_name_spec"],
                                                                             key=f"name_{idx}")
        st.session_state.api_specs[idx]["http_method"] = st.selectbox(f"HTTP Method #{idx + 1}",
                                                                      ["GET", "POST", "PUT", "DELETE"],
                                                                      index=["GET", "POST", "PUT", "DELETE"].index(
                                                                          spec["http_method"] if spec[
                                                                                                     "http_method"] in [
                                                                                                     "GET", "POST",
                                                                                                     "PUT",
                                                                                                     "DELETE"] else "GET"),
                                                                      key=f"method_{idx}")
        st.session_state.api_specs[idx]["url_path"] = st.text_input(f"URL Path #{idx + 1}", value=spec["url_path"],
                                                                    key=f"path_{idx}")
        st.session_state.api_specs[idx]["http_headers"] = st.text_area(f"HTTP Headers #{idx + 1}",
                                                                       value=spec["http_headers"], key=f"headers_{idx}")
        st.session_state.api_specs[idx]["http_body"] = st.text_area(f"HTTP Body #{idx + 1}", value=spec["http_body"],
                                                                    key=f"body_{idx}")
        st.session_state.api_specs[idx]["raw_request"] = st.text_area(f"Raw Request #{idx + 1}",
                                                                      value=spec["raw_request"], key=f"req_{idx}")
        st.session_state.api_specs[idx]["raw_response"] = st.text_area(f"Raw Response #{idx + 1}",
                                                                       value=spec["raw_response"], key=f"res_{idx}")
        # Tambah field response_code (pakai |)
        raw_codes = st.text_area(f"Response Codes #{idx + 1} (pisahkan dengan |)",
                                 value=" | ".join(spec.get("response_code", [])), key=f"resp_code_{idx}")
...
for idx, dep in enumerate(st.session_state.api_dependencies):
    with st.expander(f"Dependencies for API #{idx + 1}", expanded=True):
        st.session_state.api_dependencies[idx]["service_name_depedencies"] = st.text_input(
            f"Service Name (Dep) #{idx + 1}", value=dep["service_name_depedencies"], key=f"dep_name_{idx}")
        raw = st.text_area(f"Dependencies List (pisahkan dengan | pipe) #{idx + 1}",
                           value="|".join(dep["dependencies"]), key=f"dep_list_{idx}")
